...
def send_strategy_report(subject: str, html_body: str):
  """Synchronous worker that dispatches an HTML report directly via Gmail SMTP."""
  SMTP_SERVER = "smtp.gmail.com"
  SMTP_PORT = 587
  PRIMARY_EMAIL = "mmistroni@gmail.com"
  APP_PASSWORD = (
      os.getenv("EMAIL_PASSWORD", "").replace(" ", "") if os.getenv("EMAIL_PASSWORD") else ""
  )

  if not APP_PASSWORD:
    print(
        "⚠️ [EMAIL] EMAIL_PASSWORD environment variable is not set! Skipping"
        " email dispatch."
    )
    return
  ...
  try:
    ...
    print(
        f"❌ [EMAIL] Failed to send email via SMTP to {PRIMARY_EMAIL}: {e}"
    )


def build_summary_email_and_send(
    rows_inserted: List[Dict[str, Any]], target_date_str: str
):
  ...
  if not rows_inserted:
    ...
    subject_str = (
        f"Stock Agent Operations Report - {target_date_str} |"
        f" {len(rows_inserted)} Signals"
    )
    table_rows = ""
...
async def get_auth_token() -> str:
  ...
  try:
    async with httpx.AsyncClient() as client:
      ...
      response = await client.get(
          metadata_url, headers=headers, params=params, timeout=5.0
      )
  ...
  try:
    ...
    raise RuntimeError(
        "gcloud command not found. Please ensure Google Cloud CLI is installed."
    )
...
async def run_agent_request(
  ...
  try:
    ...
    raise json.JSONDecodeError(
        "Failed to parse standard Unary JSON response structure.",
        response.text,
        0,
    )
...
async def amain(message_to_send: str, target_date_str: str):
  ...
  print(
      "\n🤖 Starting Single-Run Stock Job | Session:"
      f" **{SESSION_ID}** | Snapshot Date: **{target_date_str}**"
  )
  ...
  async with httpx.AsyncClient(timeout=600.0) as client:
    ...
    try:
      await make_request(
          client, "POST", current_session_endpoint, data=session_data
      )
      print("✅ Session state re-initialized successfully.")
    except Exception as e:
    ...
    if agent_text:
      ...
      try:
        print(
            "🔍 [PARSING] Attempting to deserialize clean text block into"
            " JSON..."
        )
        ...
        for index, row in enumerate(raw_rows):
          ...
          action_val = str(
              row.get("signal", row.get("action", "HOLD"))
          ).upper()

          raw_score = row.get(
              "confidence_score", row.get("conviction_score", 0.0)
          )
          try:
          ...
          compiled_row = {
              ...
              "conviction_score": int(numeric_score),
              "action": action_val,
              "reasoning": str(
        ...
        if rows_to_insert:
          print(
              f"📤 [BIGQUERY] Streaming {len(rows_to_insert)} items to"
              f" {TABLE_REF}..."
          )
          ...
          if errors:
            print(
                "❌ [BIGQUERY] Ingestion errors:"
                f" {json.dumps(errors, indent=2)}"
            )
          else:
            print(
                "🎉 [BIGQUERY] Output successfully streaming ingested into"
                " BigQuery!"
            )
        else:
        ...
        print(
            "🚨 [PARSING ERROR] Agent output was not valid"
            f" JSON:\n{clean_text}"
        )
      except Exception as bq_err:
        print(
            "❌ [BIGQUERY] Error during data preparation/ingestion:"
            f" {bq_err}"
        )
      ...
      await loop.run_in_executor(
          None, build_summary_email_and_send, rows_to_insert, target_date_str
      )
...
if __name__ == "__main__":
  ...
  QUERY = (
      "Execute full schema discovery and quantitative trend analysis for market"
      f" snapshot date: {target_date_str}."
  )
